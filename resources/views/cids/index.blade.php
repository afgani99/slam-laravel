        ...
        <div class="slam-panel p-6">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-sm font-medium text-white">{{ __('cids.filter_title') }}</p>
            ...
            <form method="GET" action="{{ route('cids.index') }}" class="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-[1fr_120px_110px] lg:items-end">
                ...
                {{-- Per Page --}}
                <div class="flex flex-col gap-1.5">
                    <x-input-label for="per_page" :value="__('cids.per_page_label')" class="text-xs font-medium uppercase tracking-[0.15em] text-neutral-400" />
                    <select id="per_page" name="per_page"
                        ...
                        @foreach ([10, 25, 50] as $option)
                            <option value="{{ $option }}" @selected($perPage === $option)>{{ __('cids.per_page_option', ['count' => $option]) }}</option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm">
                    <thead class="border-b border-white/5 bg-[#2a2a2a] text-[11px] uppercase tracking-[0.18em] text-neutral-500">
                        <tr>
                            <th class="px-6 py-4 font-medium">{{ __('cids.col_cid') }}</th>
                            <th class="px-6 py-4 font-medium">{{ __('cids.col_vendor') }}</th>
                            <th class="px-6 py-4 font-medium">{{ __('cids.col_cid_is') }}</th>
                            <th class="px-6 py-4 font-medium">{{ __('cids.col_customer') }}</th>
                            <th class="px-6 py-4 font-medium">{{ __('cids.col_service') }}</th>
                            <th class="px-6 py-4 font-medium">{{ __('cids.col_target_sla') }}</th>
                            <th class="px-6 py-4 text-center font-medium">{{ __('cids.col_ticket') }}</th>
                            <th class="px-6 py-4 text-center font-medium">{{ __('cids.col_action') }}</th>
                        </tr>
                    </thead>
                            ...
                            <tr class="transition duration-150 hover:bg-white/[0.03]">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="flex justify-center items-center gap-2">
                                        ...
                                        @endif
                                        <a href="{{ route('cids.show', $cid) }}" class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-white/5 bg-[#262626] text-neutral-300 transition hover:border-white/10 hover:bg-[#2f2f2f] hover:text-white" :title="__('cids.action_detail')" :aria-label="__('cids.action_detail')">
                                            <span class="material-symbols-outlined text-[14px]">visibility</span>
                                        </a>
                                        @if(auth()->user()->role === 'admin')
                                            <form action="{{ route('cids.destroy', $cid) }}" method="POST" onsubmit="return confirm('{{ __('cids.delete_confirm', ['cid' => $cid->cid]) }}')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-white/5 bg-[#262626] text-neutral-300 transition hover:border-red-900/50 hover:bg-red-900/20 hover:text-red-400" :title="__('cids.action_delete')" :aria-label="__('cids.action_delete')">
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-6 py-16 text-center">
                                    ...
                                    <p class="text-sm text-neutral-500">{{ __('cids.no_data') }}</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
        ...
        <div x-show="showCidModal" x-transition:enter="transition duration-200 ease-out" x-transition:enter-start="translate-y-6 opacity-0 scale-[0.97]" x-transition:enter-end="translate-y-0 opacity-100 scale-100" x-transition:leave="transition duration-150 ease-in" x-transition:leave-start="translate-y-0 opacity-100 scale-100" x-transition:leave-end="translate-y-6 opacity-0 scale-[0.97]" class="relative z-10 w-full max-w-2xl">
            <div class="max-h-[calc(100vh-3rem)] overflow-hidden rounded-2xl border border-white/10 bg-[#1f1f1f] shadow-2xl shadow-black/50 sm:max-h-none">
                ...
                <div class="flex items-center gap-3 px-5 pb-2 pt-4">
                    ...
                    <h3 class="text-sm font-semibold text-white/80">{{ __('cids.modal_create_title') }}</h3>
                </div>
                ...
                <form method="POST" action="{{ route('cids.store') }}" class="max-h-[calc(100vh-8rem)] overflow-y-auto p-4 sm:max-h-none sm:overflow-visible sm:p-6">
                    ...
                    <div class="grid gap-4 md:grid-cols-2 md:gap-5">
                        ...
                        <div>
                            <x-input-label for="cid_is" :value="__('cids.label_cid_is')" />
                        ...
                        <div>
                            <x-input-label for="vendor_name" :value="__('cids.label_vendor_name')" />
                        ...
                        <div>
                            <x-input-label for="customer_name" :value="__('cids.label_customer_name')" />
                        ...
                        <div>
                            <x-input-label for="service" :value="__('cids.label_service')" />
                        ...
                        <div>
                            <x-input-label for="sla_percentage" :value="__('cids.label_sla_target')" />
        ...
        <div x-show="showEditCidModal" x-transition:enter="transition duration-200 ease-out" x-transition:enter-start="translate-y-6 opacity-0 scale-[0.97]" x-transition:enter-end="translate-y-0 opacity-100 scale-100" x-transition:leave="transition duration-150 ease-in" x-transition:leave-start="translate-y-0 opacity-100 scale-100" x-transition:leave-end="translate-y-6 opacity-0 scale-[0.97]" class="relative z-10 w-full max-w-2xl">
            <div class="rounded-2xl border border-white/10 bg-[#1f1f1f] shadow-2xl shadow-black/50">
                <div class="flex items-center gap-3 px-5 pb-2 pt-4">
                    ...
                    <h3 class="text-sm font-semibold text-white/80">{{ __('cids.modal_edit_title') }}</h3>
                </div>
                ...
                <form method="POST" :action="`/cids/${editCid.id}`" class="p-6">
                    ...
                    <div class="grid gap-5 md:grid-cols-2">
                        ...
                        <div>
                            <x-input-label for="edit_service" :value="__('cids.label_service')" />
                            <x-text-input id="edit_service" name="service" type="text" class="mt-1 block w-full" x-model="editCid.service" required />
                            <x-input-error class="mt-2" :messages="$errors->get('service')" />
                        </div>
                        <div>
                            <x-input-label for="edit_sla_percentage" :value="__('cids.label_sla_target')" />
        ...
        <div x-show="showTicketModal" x-transition:enter="transition duration-200 ease-out" x-transition:enter-start="translate-y-6 opacity-0 scale-[0.97]" x-transition:enter-end="translate-y-0 opacity-100 scale-100" x-transition:leave="transition duration-150 ease-in" x-transition:leave-start="translate-y-0 opacity-100 scale-100" x-transition:leave-end="translate-y-6 opacity-0 scale-[0.97]" class="relative z-10 w-full max-w-3xl">
            <div class="rounded-2xl border border-white/10 bg-[#1f1f1f] shadow-2xl shadow-black/50">
                ...
                <div class="flex items-center gap-3 px-5 pb-2 pt-4">
                    ...
                    <h3 class="text-sm font-semibold text-white/80">{{ __('cids.modal_ticket_title') }}</h3>
                </div>
                ...
                <form method="POST" action="{{ route('tickets.store') }}" class="p-6">
                    ...
                    <div class="space-y-5">
                        <div class="rounded-2xl border border-white/5 bg-[#262626] p-4">
                            <p class="mb-3 text-xs font-semibold uppercase tracking-[0.16em] text-neutral-500">{{ __('cids.info_cid_section') }}</p>
                            <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                                <div>
                                    <p class="text-[11px] uppercase tracking-[0.14em] text-neutral-500">{{ __('cids.label_cid') }}</p>
                                    <p class="mt-1 truncate text-sm font-medium text-white" x-text="ticketCidCid">-</p>
                                </div>
                                <div>
                                    <p class="text-[11px] uppercase tracking-[0.14em] text-neutral-500">{{ __('cids.label_cid_is') }}</p>
                                    <p class="mt-1 truncate text-sm font-medium text-white" x-text="ticketCidIs || '-'">-</p>
                                </div>
                                <div>
                                    <p class="text-[11px] uppercase tracking-[0.14em] text-neutral-500">{{ __('cids.info_vendor') }}</p>
                                    <p class="mt-1 truncate text-sm font-medium text-white" x-text="ticketCidVendor">-</p>
                                </div>
                                <div>
                                    <p class="text-[11px] uppercase tracking-[0.14em] text-neutral-500">{{ __('cids.info_customer') }}</p>
                                    <p class="mt-1 truncate text-sm font-medium text-white" x-text="ticketCidCustomer">-</p>
                                </div>
                                <div class="md:col-span-2 xl:col-span-4">
                                    <p class="text-[11px] uppercase tracking-[0.14em] text-neutral-500">{{ __('cids.info_service') }}</p>
                        ...
                        <div class="grid gap-5 md:grid-cols-2">
                            <div>
                                <x-input-label for="vendor_ticket_number" :value="__('cids.label_vendor_ticket_id')" />
                            ...
                            <div>
                                <x-input-label for="case_type" :value="__('cids.label_case_type')" />
                                <div class="relative mt-1">
                                    <select id="case_type" name="case_type" class="block h-[42px] w-full appearance-none rounded-lg border border-neutral-700 bg-neutral-900 pr-10 text-neutral-100 shadow-sm focus:border-orange-500 focus:ring-orange-500/30" required>
                                        <option value="" class="bg-neutral-900">{{ __('cids.case_type_placeholder') }}</option>
                            ...
                            <div class="md:col-span-2">
                                <x-input-label for="started_at" :value="__('cids.label_started_at')" />
