    ...
    <div class="space-y-6">
        <div class="slam-panel p-6">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-sm font-medium text-white">{{ __('sla_restitution.filter_title') }}</p>
                        <p class="text-xs text-neutral-500">{{ __('sla_restitution.filter_subtitle') }}</p>
                    </div>
                </div>
            ...
            <form method="GET" action="{{ route('sla.restitution') }}" class="mt-5 grid gap-3 lg:grid-cols-[1fr_160px_130px] lg:items-end">
                <div class="flex flex-col gap-1.5">
                    ...
                    <div class="relative">
                        ...
                        <x-text-input id="search" name="search" type="search" class="block w-full pl-10" placeholder="{{ __('sla_restitution.search_placeholder') }}" :value="request('search')" />
                    </div>
                </div>

                <div class="flex flex-col gap-1.5">
                    <x-input-label for="bulan" :value="__('sla_restitution.month')" class="text-xs font-medium uppercase tracking-[0.15em] text-neutral-400" />
                ...
                <div class="flex flex-col gap-1.5">
                    <x-input-label for="tahun" :value="__('sla_restitution.year')" class="text-xs font-medium uppercase tracking-[0.15em] text-neutral-400" />
        ...
        <div class="slam-panel overflow-hidden">
            <div class="flex items-center justify-between border-b border-white/5 px-6 py-4">
                <div>
                    <p class="text-sm font-medium text-white">{{ __('sla_restitution.list_title') }}</p>
                    <p class="text-xs text-neutral-500">{{ __('sla_restitution.list_subtitle') }}</p>
                </div>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm">
                    <thead class="border-b border-white/5 bg-[#2a2a2a] text-[11px] uppercase tracking-[0.18em] text-neutral-500">
                        <tr>
                            <th class="px-6 py-4 font-medium">{{ __('sla_restitution.cid') }}</th>
                            <th class="px-6 py-4 font-medium">{{ __('sla_restitution.vendor') }}</th>
                            <th class="px-6 py-4 font-medium">{{ __('sla_restitution.customer') }}</th>
                            <th class="px-6 py-4 font-medium">{{ __('sla_restitution.service') }}</th>
                            <th class="px-6 py-4 text-right font-medium">{{ __('sla_restitution.target') }}</th>
                            <th class="px-6 py-4 text-right font-medium">{{ __('sla_restitution.sla') }}</th>
                            <th class="px-6 py-4 text-right font-medium">{{ __('sla_restitution.downtime') }}</th>
                            <th class="px-6 py-4 text-right font-medium">{{ __('sla_restitution.pending') }}</th>
                            <th class="px-6 py-4 text-right font-medium">{{ __('sla_restitution.effective') }}</th>
                            <th class="px-6 py-4 text-center font-medium">{{ __('sla_restitution.status') }}</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-white/5">
                        @forelse ($results as $item)
                            <tr class="transition duration-150 hover:bg-white/[0.03]">
                                ...
                                <td class="px-6 py-4 text-right text-neutral-300">{{ number_format($item['total_downtime']) }} {{ __('sla_restitution.minutes') }}</td>
                                <td class="px-6 py-4 text-right text-neutral-300">{{ number_format($item['total_pending']) }} {{ __('sla_restitution.minutes') }}</td>
                                <td class="px-6 py-4 text-right text-neutral-300">{{ number_format($item['effective_downtime']) }} {{ __('sla_restitution.minutes') }}</td>
                                <td class="px-6 py-4 text-center">
                                    <span class="inline-flex rounded-full bg-red-500/10 px-3 py-1 text-xs text-red-400">
                                        {{ $item['status'] === 'Aman' ? __('sla_monthly.safe') : __('sla_monthly.not_safe') }}
                                    </span>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="10" class="px-6 py-16 text-center">
                                    ...
                                    <p class="text-sm text-neutral-400">{{ __('sla_restitution.no_data_title') }}</p>
                                    <p class="mt-1 text-xs text-neutral-500">{{ __('sla_restitution.no_data_desc') }}</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
